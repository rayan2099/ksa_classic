  ...
  <body style="margin:0;background:#111111;color:#f5f5f5;font-family:Arial,sans-serif">
    <div style="max-width:620px;margin:0 auto;padding:32px 20px">
      <div style="border-bottom:2px solid #d4af37;padding-bottom:18px;margin-bottom:28px">
        <strong style="font-size:22px;letter-spacing:1px">KSA <span style="color:#d4af37">COLLECTION</span></strong>
      </div>
      ${content}
...
const buildInquiryConfirmationEmail = (buyerName: string, carTitle: string, message: string) => ({
  ...
  `),
  text: `Thank you, ${buyerName}.\n\nWe received your inquiry${carTitle ? ` about ${carTitle}` : ''}. A member of the KSA Collection team will get back to you shortly.\n\nYour message:\n${message}`
});

const buildAdminReplyEmail = (buyerName: string, carTitle: string, reply: string, senderName: string) => ({
  subject: `KSA Collection reply${carTitle ? `: ${carTitle}` : ''}`,
  html: emailShell(`
    ...
    <p style="color:#aaa;margin-top:18px;font-size:13px">Regards,<br>${escapeHtml(senderName)}<br>KSA Collection</p>
  `),
    ...
    replies: [
      {
        ...
        message: 'Thank you for your interest in KSA Collection. Our curators are reviewing your inquiry.',
